...
def voice():
    ...
    resp.append(g)
    resp.redirect(url_for("voice", _external=True))  # fallback
    return str(resp)
...
def handle_language():
    ...
    if digit == "1":
        ...
        g.say("Спасибо, продолжаем на русском. Выберите направление: 1 — солнечные панели, 2 — тепловые насосы, 3 — другие вопросы.", language=LANG)
        resp.append(g)
        resp.redirect(url_for("handle_language", _external=True))  # fallback
    else:
...
def handle_topic():
    ...
    if digit == "1":
        ...
        g.say("Вы выбрали солнечные панели. Нажмите 1 — передать информацию в технический отдел, 2 — получить консультацию.", language=LANG)
        resp.append(g)
        resp.redirect(url_for("handle_topic", _external=True))  # fallback
        ...
        g.say("Вы выбрали тепловые насосы. Нажмите 1 — передать информацию в технический отдел, 2 — получить консультацию.", language=LANG)
        resp.append(g)
        resp.redirect(url_for("handle_topic", _external=True))  # fallback
...
def _final(theme: str, digit: str) -> VoiceResponse:
    resp = VoiceResponse()
    if digit == "1":
        resp.say("Спасибо. Ваша информация передана в технический отдел. Мы свяжемся с вами в ближайшее время.", language=LANG)
    elif digit == "2":
        resp.say(f"Я — консультант и могу рассказать о {theme}. Задайте ваш вопрос.", language=LANG)
    else:
